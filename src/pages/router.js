...
// matchURI({ path: '/posts/:id' }, '/dummy') => null   matchURI({ path:
...
// Find the route matching the specified location (context), fetch the required
// data, instantiate and return a React component
function resolve(routes, context) {
  for (const route of routes) { // eslint-disable-line no-restricted-syntax
    const params = matchURI(route, context.error
      ? '/error'
      : context.pathname);
    ...
    // Check if the route has any data requirements, for example: { path:
    // '/tasks/:id', data: { task: 'GET /api/tasks/$id' }, page: './pages/task' }
    if (route.data) {
